...
* [`src/animal_form/`](src/animal_form/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/animal_form/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `animal-form` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/animal_form/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns animal-form.example
  (:require [animal-form.config :as config])
